...
class Merlin {
    ...
    */
    function __construct( $config = array(), $strings = array() ) {
        ...
        add_action('wp_ajax_update_fetch_attachments_status', array($this, 'update_fetch_attachments_status'));
        add_option( 'electron_fetch_attachments', 'yes');
    }
    ...
    */
    ...
    protected function server_requirements($step) {
        ...
        $memoryLimit = substr(ini_get('memory_limit'),-1);
        $memory_limit = intval(substr(ini_get('memory_limit'), 0, -1));
        if ($memory_limit < 256 && $memoryLimit != 'G' ) {
            $memory_limit = 'error';
            $go = false;
        }
        ...
        <table class="widefat" cellspacing="0">
            <tbody>
                ...
                <?php if ( function_exists( 'ini_get' ) ) : ?>
                    ...
                    if ( function_exists( 'phpversion' ) ) {
                        ...
                        <?php if ($php_check == 'error'): ?>
                            <tr>
                                <td><?php esc_html_e( 'PHP Version', 'electron' ); ?></td>
                                <td>
                                    <span class="dashicons dashicons-warning red"></span> <span><?php echo esc_html($phpversion); ?></span>
                                    <div><small><?php esc_html_e('Recommended: ', 'electron'); ?><strong>7.0 or higher</strong></small></div>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php	} ?>
                    ...
                    <?php if ($max_input_vars == 'error'):
                        ...
                        ?>
                        <tr>
                            <td><?php esc_html_e( 'max_input_vars', 'electron' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning <?php echo esc_attr($color); ?>"></span> <span><?php echo ini_get('max_input_vars'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'electron'); ?><strong>3000</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($memory_limit == 'error'): ?>
                        <tr>
                            <td><?php esc_html_e( 'memory_limit', 'electron' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('memory_limit'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'electron'); ?><strong>256M</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($max_execution_time == 'error'): ?>
                        <tr>
                            <td><?php esc_html_e( 'max_execution_time', 'electron' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('max_execution_time'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'electron'); ?><strong>300</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($upload_max_filesize == 'error'): ?>
                        <tr>
                            <td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'electron' ); ?>:</td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo size_format( wp_max_upload_size() ); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'electron'); ?><strong>12M</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                ...
                $posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'electron' ) . '">[?]</a>';
        ...
        <?php if ( $go == false) { ?>
            <?php if ( $step == 'welcome' ) : ?>
                <p class="server_check_notice step_welcome red"><?php echo sprintf( __( 'Your server does not match the recommended settings. You might not be able to import the full demos.', 'electron' ), '<span class="underline">', '</span>' ); ?></p>
                <p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s, or proceed with caution :(', 'electron' ), '<a href="https://ninetheme.com/docs/wordpress-server-requirements/" target="_blank">', '</a>' ); ?></p>
            <?php else: ?>
                <p class="server_check_notice step_import red"><?php echo sprintf( __( 'Your server does not match the recommended settings. %sImport the BASIC Demo%s only, or adjust your settings.', 'electron' ), '<span class="underline">', '</span>' ); ?></p>
            <?php endif; ?>
        <?php } ?>
