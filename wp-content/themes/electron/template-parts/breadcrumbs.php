...
if ( ! function_exists( 'electron_breadcrumbs' ) ) {
    ...
    function electron_breadcrumbs( $args = array() ) {

        $breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );
        $breadcrumbs_type = electron_settings( 'breadcrumbs_type', 'theme' );

        if ( '0' == electron_settings( 'breadcrumbs_visibility', '1' ) )
        return;

        if ( class_exists( 'WooCommerce' ) && 'woo' == $breadcrumbs_type ) {
            return woocommerce_breadcrumb();
        } elseif ( function_exists( 'rank_math_the_breadcrumbs' ) && 'rank' == $breadcrumbs_type ) {
            return rank_math_the_breadcrumbs();
        } elseif ( function_exists( 'yoast_breadcrumb' ) && 'yoast' == $breadcrumbs_type ) {

            return yoast_breadcrumb('<nav class="yoast-breadcrumb">','</nav>');
        }

        if ( ! is_object( $breadcrumb ) )
        $breadcrumb = new Electron_Breadcrumb( $args );

        return $breadcrumb->trail();
    }

    add_filter( 'woocommerce_breadcrumb_defaults', 'goldsmith_change_breadcrumb_delimiter' );
    function goldsmith_change_breadcrumb_delimiter( $defaults ) {
        $defaults['delimiter'] = '';
        return $defaults;
    }
}
...
class Electron_Breadcrumb {
    ...
    */
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            $breadcrumb .= sprintf(
                '<%s class="trail-items electron-breadcrumb">',
            ...
            $breadcrumb = sprintf(
                '<%1$s aria-label="%2$s" class="breadcrumb-trail electron-breadcrumbs">%3$s%4$s%5$s</%1$s>',
    ...
    */
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'electron' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'electron' ),
            'home'                => esc_html__( 'Home',                                  'electron' ),
            'error_404'           => esc_html__( '404 Not Found',                         'electron' ),
            'archives'            => esc_html__( 'Archives',                              'electron' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'electron' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'electron' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'electron' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'electron' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'electron' ),
    ...
    */
    protected function add_minute_hour_archive_items() {
        ...
        $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'electron' ) ) );
    }
    ...
    */
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
        $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'electron' ) ) );
    }
    ...
    */
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
        $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'electron' ) ) );
    }
    ...
    */
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'electron' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'electron' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'electron' ) ) );
    ...
    */
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'electron' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'electron' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'electron' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag && true === $this->args['show_month'] )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'electron' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag && true === $this->args['show_day'] )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'electron' ) ) ) );
